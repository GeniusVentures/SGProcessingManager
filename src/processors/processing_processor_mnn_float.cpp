...
namespace sgns::sgprocessing
{
                                                ...
                                                const ExecutionContext            &execCtx )
    {
        ...
        {
            m_logger->error( "Float input missing width" );
        ...
        {
            m_logger->error( "Invalid float length/patch/stride values" );
            return ProcessingResult{};
        }
        ...
        {
            m_logger->error( "Float supports FLOAT32/FLOAT16 formats only" );
            return ProcessingResult{};
        }
        ...
        if ( floatData.size() < expectedBytes )
        {
            m_logger->error( "Float input size {} bytes is smaller than expected {} bytes",
                             floatData.size(),
                             expectedBytes );
            return ProcessingResult{};
        }
        ...
        {
            const auto *src = reinterpret_cast<const float *>( floatData.data() );
            std::memcpy( signalValues.data(), src, expectedBytes );
        }
        else
        {
            const auto *src = reinterpret_cast<const uint16_t *>( floatData.data() );
        ...
        m_logger->info( "Processing float input length: {} | patch: {} | stride: {}", length, patchLength, stride );
        ...
        {
            RunTeardown();
            return ProcessingResult{ {}, nullptr, {}, ProcessingError{ ProcessingErrorStage::CANCELLED, "Float pass cancelled" } };
        }
        ...
        m_logger->info( "Float processing complete" );
    ...
    std::unique_ptr<MNN::Tensor> MNN_Float::Process( const std::vector<float> &signalData,
                                                       std::vector<uint8_t>    &modelFile,
                                                       int                      length )
    {
        auto interpreter = std::shared_ptr<MNN::Interpreter>( MNN::Interpreter::createFromBuffer( modelFile.data(), modelFile.size() ) );
        if ( !interpreter )
        {
